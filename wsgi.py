...
db = TinyDB('arvora1.json')

# config options
define('port', default=8000, type=int, help='port to run web server on')
...
class KnowledgeHandler(tornado.web.RequestHandler):
    ...
    def post(self, *_):
        ...
        # if data:
        #     db.insert(data)
        # KnowledgeHandler.articles = {att: val for att, val in data.items() if att != "articles"}
        with open("db.json", "w") as fjson:
...
application = tornado.web.Application([
    ...
    (r'api/load', KnowledgeHandler),
    (r'api/save', KnowledgeHandler)
], debug=True)
